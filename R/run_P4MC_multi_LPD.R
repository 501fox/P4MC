...
run_P4MC_multi_LPD <- function(datasets, k = 10, S = 200, grid = 25,
                               seed = 123,
                               calculate_auc = FALSE,
  ...
  for (i in seq_len(k)) {
    ...
    marg_base <- dimreduce_and_logit_LPD(train = train, isscale = TRUE, classifier = classifiers[1])
    marg0 <- marg_base[[1]]
    marg1 <- marg_base[[2]]
    ...
    for (j in 1:nrow(test_scaled)) {
      ...
      mcmc_samples_y0[[j]] <- p4mc_kde_sample_LPD(newX1, target_y = 0, S, grid, kde_y0, kde_y1)
      mcmc_samples_y1[[j]] <- p4mc_kde_sample_LPD(newX1, target_y = 1, S, grid, kde_y0, kde_y1)
    }
    ...
    for (clf in classifiers) {
      if (clf == classifiers[1]) {
        ...
        marg_temp <- dimreduce_and_logit_LPD(train = train, isscale = TRUE, classifier = clf)
        fit_model <- marg_temp[[3]]
      }
      
      # Calculate class priors for inference
      pi1 <- mean(as.numeric(as.character(train$Y)) == 1)
      pi0 <- 1 - pi1
      
      P_score0 <- sapply(1:nrow(test_scaled), function(j) {
        get_prediction_P4MC_LPD(j, test_scaled, mcmc_samples_y0[[j]], mcmc_samples_y1[[j]], fit_model, clf, pi0=pi0, pi1=pi1)
      })
      ...
      # Determine classification threshold dynamically based on training set class prior
      prior_threshold <- mean(as.numeric(as.character(train$Y)) == 1)
      pred_class <- ifelse(P_score1 >= prior_threshold, 1, 0)
