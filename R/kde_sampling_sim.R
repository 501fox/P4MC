...
p4mc_kde_sample_sim <- function(pre_data, target_y, S, grid, kde_y0, kde_y1){
  
  # 1. Select the corresponding KDE object based on target class
  kde <- if (target_y == 0) kde_y0 else kde_y1
  x <- kde$eval.points[[1]] # Grid of the core latent variable R(X2)
  ...
  err <- sapply(3:10, function(k){
    ...
    sum(abs(y - predict(fit_temp, data.frame(x))))
  })
  ...
  # Fallback: Prevent rnormTrunc error if min equals max
  if (min_val >= max_val) {
    min_val <- min(x)
    max_val <- max(x)
  }
  ...
  for (s in seq_len(S)) {
    ...
    theta.star <- EnvStats::rnormTrunc(n = 1, mean = theta, sd = 0.6, 
    ...
    # Calculate M-H acceptance ratio (Intercepting negative values from B-spline)
    if (is.na(p_theta_star) || p_theta_star <= 0) {
      r <- 0
    } else if (is.na(p_theta) || p_theta <= 0) {
      r <- 1 # Force accept if current point is invalid
    } else {
      # Retain original acceptance ratio formula
  ...
  # Assign smoothed probability density to each sampled point using B-spline
  P <- predict(fit, data.frame(x = RX2_sampled))
